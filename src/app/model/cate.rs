...
use crate::app::entity::{cate, cate::Entity as Cate};
...
impl CateModel {
    ...
    // 搜索
    pub async fn search_count(db: &DbConn, wheres: CateWhere) -> Result<u64, DbErr> {
        Cate::find()
    ...
    pub async fn find_all(db: &DbConn) -> Result<Vec<cate::Model>, DbErr> {
        Cate::find()
            .order_by_desc(cate::Column::Sort)
            .order_by_asc(cate::Column::Id)
            .all(db)
    ...
    /// 查询启用的分类
    pub async fn find_open_cate(db: &DbConn) -> Result<Vec<cate::Model>, DbErr> {
        Cate::find()
            .order_by_desc(cate::Column::Sort)
            .order_by_asc(cate::Column::Id)
    ...
    pub async fn create(db: &DbConn, form_data: cate::Model) -> Result<cate::ActiveModel, DbErr> {
        cate::ActiveModel {
