...
use crate::nako::global::{AppState, Deserialize, Serialize, Session};
use crate::nako::{app, auth as nako_auth, http as nako_http, time};
...
pub async fn list(
    ...
    let (list, _num_pages) =
        user::UserModel::search_in_page(db, page, per_page, search_where.clone())
            .await
            .unwrap_or_default();
...
pub async fn detail(
    ...
    let user_data = user::UserModel::find_user_by_id(db, query.id)
        .await
        .unwrap_or_default()
        .unwrap_or_default();
    if user_data.id == 0 {
...
pub async fn create_save(
    ...
    let user_data = user::UserModel::find_user_by_name(db, params.username.as_str())
        .await
        .unwrap_or_default()
        .unwrap_or_default();
    if user_data.id > 0 {
    ...
    let create_data = user::UserModel::create_user(
        db,
        user_entity::Model {
...
pub async fn update(
    ...
    let user_info = user::UserModel::find_user_by_id(db, query.id)
        .await
        .unwrap_or_default()
        .unwrap_or_default();
    if user_info.id == 0 {
...
pub async fn update_status(
    ...
    // 更新
    let user_data = user::UserModel::update_status_by_id(
        db,
        query.id,
        user_entity::Model {
            status: Some(params.status),
...
pub async fn update_password(
    ...
    Ok(nako_http::view(
        &mut view,
        "admin/user/update_password.html",
        &ctx,
    ))
}
