...
use crate::app::entity::{self, page as page_entity};
use crate::app::model::{page, user};
...
pub async fn list(
    ...
    let (list, _num_pages) =
        page::PageModel::search_in_page(db, page, per_page, search_where.clone())
            .await
            .unwrap_or_default();
...
pub async fn detail(
    ...
    let data = page::PageModel::find_by_id(db, query.id)
        .await
        .unwrap_or_default()
        .unwrap_or_default();
    if data.id == 0 {
...
pub async fn create_save(
    ...
    let data = page::PageModel::find_by_slug(db, params.slug.as_str())
        .await
        .unwrap_or_default()
        .unwrap_or_default();
    if data.id > 0 {
    ...
    let create_data = page::PageModel::create(
        db,
        page_entity::Model {
            user_id: user_id,
...
pub async fn update(
    ...
    let info = page::PageModel::find_by_id(db, query.id)
        .await
        .unwrap_or_default()
        .unwrap_or_default();
    if info.id == 0 {
...
pub async fn update_save(
    ...
    let info_by_name = page::PageModel::find_by_slug(db, params.slug.as_str())
        .await
        .unwrap_or_default()
        .unwrap_or_default();
    ...
    // 更新
    let data = page::PageModel::update_by_id(
        db,
        query.id,
        page_entity::Model {
            slug: params.slug.clone(),
...
pub async fn update_status(
    ...
    // 更新
    let status = page::PageModel::update_status_by_id(
        db,
        query.id,
        page_entity::Model {
            status: Some(params.status),
